...
describe Deep::Ignore::Order::Matcher do
  it 'should matches usual values' do
    ['an_string', 1, 13.5, nil, [1, 2, 3], { a: 1, b: 2 }].each_slice(2) do |value1, value2|
      expect(value1).to deep_equal value1
      expect(value2).to deep_equal value2
      expect(value1).to_not deep_equal value2
      expect(value2).to_not deep_equal value1
    end
  end
  ...
  it 'should match deep structs' do
    ...
    expect(actual).to deep_equal expected
    actual[0][:c].push(4)
    expect(actual).to_not deep_equal expected
  end

  it 'should do not match partials' do
    expect([1, 2, 3]).to_not deep_equal [1, 2]
    expect([1, 2]).to_not deep_equal [1, 2, 3]
    expect(a: 1, b: 2).to_not deep_equal(a: 1)
    expect(a: 1).to_not deep_equal(a: 1, b: 2)
  end

  it 'should ignore hash keys order' do
    expect(a: 1, b: 2).to deep_equal(b: 2, a: 1)
  end
end
